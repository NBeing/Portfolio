...
router.use(function(req, res, next) {
    // do logging
    res.header("Access-Control-Allow-Credentials", "true") // Be an insecure bad boy because the data isnot volatile (it can be rescraped) 
...
router.route('/hegels')
    ...
    .post(function(req, res) {  // This runs the web scraping function
        ...
        scraper.get_toc(url)  //Get the TOC from url
            ...
            .then(function(data){
                return scraper.get_multiple(data); //Get all of the pages from the TOC links
            })
            .then(function(data){
...
//Find a single section in the mongodb database
...
//This is so dumb and impure why are you doing this. Fix soon.
var wikiQuery = [];
...
router.route('/all/:id')
    .get(function(req, res){
        ...
        Hegel.findOne({'number':id}, function(err, hegel ){
            ...
            try{
                const hegelText = tokenizer.tokenize(hegel.hegel.text[0])
                const findlayText = hegel.findlay.text[0]
                const together = {
                    hegel: hegelText,
                    findlay: findlayText
                }
